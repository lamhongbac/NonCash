...
| CR ID | Item | Status |
|---|---|---|
| CR-2026-09-06-12 | Reword the `VoucherReceived` template with calmer transactional copy — the current promotional phrasing ("You've received a voucher") is a spam-filter fingerprint risk (observed during the 2026-09-06 deliverability incident) | **Implemented** 2026-09-11 — subject is now `Your NonCash voucher: {name}` (no name → `Your NonCash voucher`), heading `Your voucher is in your NonCash wallet`, neutral header band, and the stale "Log in with your phone number" line corrected to phone **or** email per CR-27. Placeholder set unchanged (`RecipientName`, `VoucherName`, `FaceValue`, `ExpiryDate`, `PhoneNumber`, `MagicLinkHtml`), so no caller moved. Inbox placement (Primary vs Promotions) still needs the owner's own manual check against the two authorized addresses |
| CR-2026-09-06-13 | Email sender-account recovery after the 2026-09-06 compromise: revoke the leaked Gmail app password → change the account password → check filters/forwarding → issue a new app password injected via env var (never committed) → rotate the other secrets sharing that file (DB, ZaloPay, VNPAY, MediaService) → set up SPF/DKIM/DMARC for the sender domain → 24–72h cool-down → retest to the two authorized addresses only. Owner: user (Google account); agent assists on config/env vars | **Partly done 2026-09-11.** *Done:* the replacement app password is no longer committed anywhere — it lives only in `%APPDATA%\Microsoft\UserSecrets\745a52d9-…\secrets.json`, and real delivery to the two authorized addresses was verified working. Every other credential that shared those files (`Jwt:Key`, the four connection strings, MSA `ApiKey`/`AppCode`, `VNPAY:TmnCode`/`HashSecret`, `ZaloPay:Key1`/`Key2`) is now out of all four tracked `appsettings*.json` files, the CI workflow, four docs, a generated wiki page and `tools/DbQuery`, with both apps holding their own `UserSecretsId`. *Still owed, and only the owner can do it:* revoke the leaked app password, change the Google account password and check filters/forwarding; **rotate** the DB, ZaloPay, VNPAY and MediaService credentials at their providers — moving them out of the working tree does not remove them from git history, so all of them must be treated as exposed; SPF/DKIM/DMARC for the sender domain; and the inbox-placement check (Primary vs Promotions) for the rewritten `VoucherReceived` copy. See `deployment-guide.md` §8 and `customer-action-matrix.md` §9 |
